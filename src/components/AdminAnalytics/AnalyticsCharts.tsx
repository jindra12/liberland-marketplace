    ...
    <>
      ...
      <div className={styles.secondaryGrid}>
        <section className={styles.card}>
          <div className={styles.cardHeader}>
            <h2>Top Queries</h2>
            <p>Most frequent GraphQL query operations seen by local analytics.</p>
          </div>
          {topQueries.length === 0 ? (
            <div className={styles.emptyChart}>No GraphQL queries have been tracked yet.</div>
          ) : (
            <div className={styles.chart}>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={topQueries} layout="vertical" margin={{ left: 8, right: 12 }}>
                  <CartesianGrid stroke="var(--theme-elevation-100)" strokeDasharray="3 3" />
                  <XAxis allowDecimals={false} type="number" />
                  <YAxis
                    dataKey="label"
                  ...
                  />
                  <Tooltip contentStyle={tooltipStyle} />
                  <Bar dataKey="count" fill="#0f766e" name="Queries" radius={[0, 10, 10, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          )}
        ...
        <section className={styles.card}>
          <div className={styles.cardHeader}>
            <h2>Top Mutations</h2>
            <p>Most frequent GraphQL mutation operations seen by local analytics.</p>
          </div>
          {topMutations.length === 0 ? (
            <div className={styles.emptyChart}>No GraphQL mutations have been tracked yet.</div>
          ) : (
            <div className={styles.chart}>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={topMutations} layout="vertical" margin={{ left: 8, right: 12 }}>
                  <CartesianGrid stroke="var(--theme-elevation-100)" strokeDasharray="3 3" />
                  <XAxis allowDecimals={false} type="number" />
                  <YAxis
                    dataKey="label"
                  ...
                  />
                  <Tooltip contentStyle={tooltipStyle} />
                  <Bar dataKey="count" fill="#f97316" name="Mutations" radius={[0, 10, 10, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          )}
