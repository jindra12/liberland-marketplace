...
7. **Backports**: Do not spend time on backports unless the user explicitly asks for them.
...
- Do not stop work early if TypeScript or the build still reports errors; keep fixing the underlying issues until the relevant build or typecheck exits successfully.
...
- Never manually edit autogenerated files such as `src/payload-types.ts`; fix the source config and regenerate them instead.
...
- Do not hide simple selection or fallback logic inside an inline IIFE when a named helper would make the flow clearer and easier to test.
...
- Never use `as never`, `as unknown`, or `as any`. This is an absolute prohibition, not a style preference. Do not coerce types unless there is literally no other choice and the coercion is unavoidable.
- Do not create ad-hoc `.d.ts` files to paper over missing module typing. Fix the source import path, package typings, or runtime entrypoint instead.
...
- Never use `function` declarations, `function()` expressions, or `export async function` unless there is an absolute technical need; use arrow functions everywhere else.
- Do not use useless expressions like `=== true` or `=== false` unless there is an explicit need for it; prefer the direct boolean expression instead.
- Never use `Object.prototype` in application code; use normal property checks like `'key' in obj` instead.
- Do not use `Reflect.get` in application code when normal typed property access or `'key' in obj` checks are sufficient.
